 ...
 *
 * Pins one release manifest for the page lifetime, loading its seven baked bundles lazily.
 * Every view reads the same validated snapshot; the deployed site needs no backend or database.
 */
...
export interface Manager {
  ...
  /** "National trophies": titles won as a NATIONAL COACH across the senior/youth World Cups and
   *  every registered regional cup (see sync/worldCup.ts and sync/officialTournaments.ts). The
...
export interface Winner {
  ...
  club: string;
  manager: string;
  /** Manager's nationality (country name, matches NATIONALITY_ISO in flags.ts), joined by exact
   *  userId when available. Login-name matching is retained only for legacy rows without an id. */
  nationality?: string;
...
interface RawLeague {
  leagueId: number;
  country: string;
  champions: Array<{ season: number; club: string; manager: string; teamId?: number; userId?: number }>;
}

export interface NationalityManagerIdentity {
  userId: number;
  userName: string;
  nationality: string;
}

export interface WinnerManagerIdentity {
  userId?: number;
  manager: string;
}

/** Resolve a winner's nationality without allowing a stale/reused login name to override an exact
 * Hattrick user identity. Name matching exists solely for old baked rows that predate userId. */
export function createManagerNationalityResolver(managers: readonly NationalityManagerIdentity[]) {
  const natById = new Map(managers.map((m) => [m.userId, m.nationality]));
  const natByLogin = new Map(managers.map((m) => [m.userName, m.nationality]));
  return (winner: WinnerManagerIdentity): string | undefined => {
    if (Number.isSafeInteger(winner.userId) && winner.userId! > 0) return natById.get(winner.userId!);
    return natByLogin.get(winner.manager);
  };
}
...
function load() {
  if (!bundle) {
    bundle = Promise.all([
      snapshot.load<RawManager[]>('managers.json'),
      snapshot.load<RawLeague[]>('leagues.json'),
...
// large, so we don't pull it into the initial bundle. Only an old, manifest-less bake may omit it.
let cupsBundle: Promise<CupCountry[]> | null = null;
function loadCups() {
  if (!cupsBundle) {
    cupsBundle = snapshot.load<CupCountry[]>('cups.json', []);
  }
  return cupsBundle;
}
...
function loadSeasonal() {
  if (!seasonalBundle) {
    seasonalBundle = snapshot.load<RawSeasonalCup[]>('seasonal.json', []);
  }
...
export async function getSeasonalCups(): Promise<SeasonalCupRoll[]> {
  ...
  const nationalityOf = createManagerNationalityResolver(managers);
  return raw.map((c) => ({
    cupId: c.cupId,
    cupName: c.cupName,
    isGeneration: c.cupId !== SUPPORTER_WEEK_CUP_ID,
    winners: c.winners.map((w) => ({ ...w, nationality: nationalityOf(w) })),
  }));
}
...
function loadMasters() {
  if (!mastersBundle) {
    mastersBundle = snapshot.load<Winner[]>('masters.json', []);
  }
...
export async function getMastersWinners(): Promise<Winner[]> {
  ...
  const nationalityOf = createManagerNationalityResolver(managers);
  return raw.map((w) => ({ ...w, nationality: nationalityOf(w) }));
}
...
export async function getCups(leagueId: string): Promise<CupRoll[]> {
  ...
  if (!country) return [];
  const nationalityOf = createManagerNationalityResolver(managers);
  return country.cups.map((c) => ({ ...c, winners: c.winners.map((w) => ({ ...w, nationality: nationalityOf(w) })) }));
}
...
export async function getWinners(leagueId: string): Promise<Winner[]> {
  ...
  if (!l) return [];
  const nationalityOf = createManagerNationalityResolver(managers);
  return l.champions.map((c) => ({ ...c, nationality: nationalityOf(c) }));
}

/**
 * World Cup (senior + youth) — the champion is a NATION, not a manager/club, so it's a completely
 * separate shape from Winner. The retained history originated on World/WorldCup/History.aspx;
 * new-format editions are monitored through official tournament XML (see
 * server/sync/officialTournaments.ts). The youth bracket was "U20" through edition 31, "U21" after.
 */
export interface WorldCupEdition {
  ...
  thirdFourth: string[];
  /** Set when retained fixture evidence carries exact national-team identities; absent on older
   *  name-only World Cup history. Flags fall back to the nation name there. */
...
export async function getWorldCup(): Promise<WorldCupHistory> {
  if (!worldCupBundle) {
    worldCupBundle = snapshot.load<WorldCupHistory>('worldcup.json', EMPTY_WC);
  }
 ...
 *
 * Older name-only World Cup editions can therefore show golds but no coach medals. Modern official
 * podium results still wait for time-correct coach evidence before any person receives a medal.
 */
 ...
 * given World Cup cycle, independent of whether that country ever won anything. No complete CHPP
 * election-history path exists; permitted retained captures are reviewed per country (see
 * server/sync/elections.ts and update/review.ts).
...
function loadElections() {
  if (!electionsBundle) {
    electionsBundle = snapshot.load<ElectionResult[]>('elections.json', []);
  }
